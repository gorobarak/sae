...
def construct_prompt(topk_samples):
    prefix="""
You are given the top 10 text samples that most strongly activated a specific feature in a Sparse Autoencoder (SAE) trained on natural language. Activation values range from 0 (no activation) to 10 (strongest possible activation).
Your task: Write a single, clear sentence that describes the common theme, pattern, or concept captured by this feature—one that explains why it activated across these samples. Try to cover as many of the examples as possible.
Samples (with activation values):
"""
    prompt_parts = [prefix]
    ...
    postfix="""
Output format:
One short sentence summarizing what this feature detects in text. No need to explain the activation values or individual samples. 
"""
