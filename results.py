...
projects = ['baseline_with_and_without_norm_decoder_v1']
...
for bool_val in [True, False]:
    for sae_type in ['topk']:
        data = df[
            ...
            (df['normlized_decoder_weights'] == bool_val)
                    ]
        data = data.sort_values(by='l0_norm')
        label_suffix = '_normalized' if bool_val else '_not_normalized'
        axs[0, 1].plot(data['l0_norm'], data['normalized_mse'], 
                    marker='o', linestyle='--', label= sae_type + label_suffix)
...
for project in projects:
    for sae_type in ['butterfly_topk', 'topk']:
        ...
        data = data.sort_values(by='l0_norm')
        label_suffix = '_new_aux' if 'new' in project else '_old_aux'
        axes.plot(data['l0_norm'], data['runtime_mins'], 
                    marker='o', linestyle='--', label=sae_type+label_suffix)
axes.set_title('runtime vs k (dictionary size = 12288)')
axes.set_xlabel('k')
